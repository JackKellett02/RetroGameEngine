...
Button::Button()
{
	m_v2UnmodifiedPos = RTMATH::Vector2();
	m_v2Pos = RTMATH::Vector2();
	m_v2Dimensions = RTMATH::Vector2(Graphics::SCREEN_WIDTH, Graphics::SCREEN_HEIGHT);
	m_bIsSelected = false;
	m_tTexture = nullptr;
...
{
	m_v2UnmodifiedPos = RTMATH::Vector2(a_fX, a_fY);
	m_v2Pos = RTMATH::Vector2(a_fX - a_fWidth / 2, a_fY - a_fHeight / 2);
	m_v2Dimensions = RTMATH::Vector2(a_fWidth, a_fHeight);
	m_bIsSelected = false;
...
Button::Button(RTMATH::Vector2 a_v2Pos, RTMATH::Vector2 a_v2Dimensions, Texture* a_tTexture, Texture* a_tTextTexture, TextureManager* a_tTextureManager, Command* a_cCommand)
{
	m_v2UnmodifiedPos = a_v2Pos;
	m_v2Pos = RTMATH::Vector2(a_v2Pos.x - a_v2Dimensions.x / 2, a_v2Pos.y - a_v2Dimensions.y / 2);
...
RTMATH::Vector2 Button::GetUnmodifiedPos()
{
...
RTMATH::Vector2 Button::GetPos()
{
	return m_v2Pos;
}

//Get the current dimensions of the button.
RTMATH::Vector2 Button::GetDimensions()
{
...
void Button::Update()
{
	RTMATH::Vector2 modifiedDimensions = m_v2Dimensions;
	RTMATH::Vector2 modifiedPos = m_v2Pos;
	if (m_bIsSelected)
	{
